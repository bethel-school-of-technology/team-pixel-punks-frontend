...
const Locations = () => {
  ...
  useEffect(() => {
    ...
    //call API.getUserLocations(), get info from backend and assign to userLocations
    //loop through the array userLocations
    //add the location data to the completeLocation object
    //get weather information using the location's lat long
    //depending on the temperature, set the completeLocation object's alert attribute to the correct svg   
    ...
    API.getUserLocations()
      .then(res => {
        ...
        for (let i = 0; i < userLocations.length; i++) {
          ...
          API.getWeatherData(userLocations[i].Latitude, userLocations[i].Longitude).then(res => {
            ...
            if (res.data.daily[0].temp.max > 75) {
              ...
              completeLocation.Alert = (<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="orange" className="bi bi-sun" viewBox="0 0 16 16">
              ...
              console.log(completeLocation.Alert);
            } else if (res.data.daily[0].temp.min < 35) {
              console.log('cold');
              completeLocation.Alert = (<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="blue" className="bi bi-snow2" viewBox="0 0 16 16">
            ...
            console.log(fullLocationsList[i]);
          });
        };
