...
const Locations = () => {
      ...
      API.getUserLocations().then((res) => {
        ...
        for (let i = 0; i < userLocations.length; i++) {
          ...
          API.getWeatherData(
            ...
            fullLocationsList.push(completeLocation)
          });
        }
        setTimeout(() => {
  ...
  const handleSubmit = (event) => {
    event.preventDefault();
     API.addLocation({ userId: user.UserId, zipcode: newLocation.zipcode }).then(() =>{
  ...
  const handleDelete = (id) => {
    API.deleteLocation(id).then((response) => {
        ...
        <div className="container-lg">
          <div className="text-center">
            ...
            <div className="p-2 bg-primary bg-opacity-10 border border-black border-opacity-0 border-5 rounded-3">
              ...
              {locations === null ? <div>no locations</div> :
